...
app.post('*', (req, res, next) => {
  console.log('[INFO] Incomming POST request');
  next();
});

app.post('/cautare/dosare', (req, res) => {
  soap.createClient(URL, (err, client) => {
    client.CautareDosare({ numeParte: 'Ghercea Paul' }, (err, result) => {
      if (err) {
      ...
      if (result) {
        res
          .status(200)
          .send({ result, count: result.CautareDosareResult.Dosar.length });
      }
    });
  });
});
...
app.post('/cautare/sedinte', (req, res) => {
  ...
  soap.createClient(URL, (err, client) => {
    client.CautareSedinte({ numeParte: 'Ghercea Paul' }, (err, result) => {
      if (err) {
        return res.status(503).send({ error: err });
      }
      if (result) {
